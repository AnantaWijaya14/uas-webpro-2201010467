...
if (isset($_POST["act"])) {
    ...
    switch ($act) {
        case "store":
            ...
            $idgeek = md5($umur);
            $sql = "INSERT INTO tbgeek(nama, nickname, umur, role, jeniskelamin, negara, idgeek) VALUES('$nama','$nickname', '$umur', '$role', '$jeniskelamin','$negara', '$idgeek');";
        ...
        case "update":
            ...
            $negara = $_POST["txNEGARA"];
            $idgeek = $_POST["idgeek"];
            $sql = "UPDATE tbgeek SET nama='$nama', nickname='$nickname', umur='$umur', role ='$role' , jeniskelamin ='$jeniskelamin', negara = '$negara' WHERE idgeek='$idgeek';";
            $hasil = mysqli_query($cnn, $sql);
            if ($hasil) {
                $footer = "<script>
                        Swal.fire({
        ...
        case "destroy":
            $idgeek = $_POST['idgeek'];
            $sql = "DELETE FROM tbgeek WHERE idgeek='$idgeek';";
            $hasil = mysqli_query($cnn, $sql);
            if ($hasil) {
                $footer = "<script>
                        Swal.fire({
...
if (isset($_GET["aksi"])) {
    ...
    switch ($aksi) {
        case "new":
            $jdpage = "Tambah";
            $pg = "team/geek/newgeek.php";
            break;
        case "edit":
            $jdpage = "Ubah";
            $pg = "team/geek/editgeek.php";
            break;
        case "del":
            $jdpage = "Hapus";
            $pg = "team/geek/delgeek.php";
            break;
        default:
            $jdpage = "List";
            $pg = "team/geek/listgeek.php";
...
<head>
    ...
    <title><?= $jdpage ?> - Data Pemain geek</title>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Data Pemain Geek</h1>
                </div>
                <?php
                include($pg);
                ?>
