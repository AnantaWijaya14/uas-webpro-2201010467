...
if (isset($_POST["act"])) {
    ...
    switch ($act) {
        case "store":
            ...
            $idonic = md5($umur);
            $sql = "INSERT INTO tbonic(nama, nickname, umur, role, jeniskelamin, negara, idonic) VALUES('$nama','$nickname', '$umur', '$role', '$jeniskelamin','$negara', '$idonic');";
        ...
        case "update":
            ...
            $negara = $_POST["txNEGARA"];
            $idonic = $_POST["idonic"];
            $sql = "UPDATE tbonic SET nama='$nama', nickname='$nickname', umur='$umur', role ='$role' , jeniskelamin ='$jeniskelamin', negara = '$negara' WHERE idonic='$idonic';";
            $hasil = mysqli_query($cnn, $sql);
            if ($hasil) {
                $footer = "<script>
                        Swal.fire({
        ...
        case "destroy":
            $idonic = $_POST['idonic'];
            $sql = "DELETE FROM tbonic WHERE idonic='$idonic';";
            $hasil = mysqli_query($cnn, $sql);
            if ($hasil) {
                $footer = "<script>
                        Swal.fire({
...
if (isset($_GET["aksi"])) {
    ...
    switch ($aksi) {
        case "new":
            $jdpage = "Tambah";
            $pg = "team/onic/newonic.php";
            break;
        case "edit":
            $jdpage = "Ubah";
            $pg = "team/onic/editonic.php";
            break;
        case "del":
            $jdpage = "Hapus";
            $pg = "team/onic/delonic.php";
            break;
        default:
            $jdpage = "List";
            $pg = "team/onic/listonic.php";
...
<head>
    ...
    <title><?= $jdpage ?> - Data Pemain onic</title>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Data Pemain Onic</h1>
                </div>
                <?php
                include($pg);
                ?>
