<?php
$idbtr = $_GET["p1"];
$sql = "SELECT tu.nama,tu.nickname, tu.umur, tu.role, tu.jeniskelamin, tu.negara FROM tbbtr tu WHERE tu.idbtr='$idbtr';";
$hasil = mysqli_query($cnn, $sql);
if (mysqli_num_rows($hasil) > 0) {
    $h = mysqli_fetch_assoc($hasil);
?>
    <h3>Hapus Data <?= $h["nama"] ?></h3>
    <form method="POST" action="databtr.php">
        <input type="hidden" name="act" value="destroy">
        <input type="hidden" name="idbtr" value="<?= $idbtr ?>">
        ...
        <a href="databtr.php" class="btn btn-secondary"> Batal </a>
    </form>
<?php
} else {
    echo "<script>window.location.href='databtr.php';</script>";
}
?>