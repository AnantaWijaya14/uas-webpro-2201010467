<?php
$idrbl = $_GET["p1"];
$sql = "SELECT tu.nama,tu.nickname, tu.umur, tu.role, tu.jeniskelamin, tu.negara FROM tbrbl tu WHERE tu.idrbl='$idrbl';";
$hasil = mysqli_query($cnn, $sql);
if (mysqli_num_rows($hasil) > 0) {
    $h = mysqli_fetch_assoc($hasil);
?>
    <h3>Hapus Data <?= $h["nama"] ?></h3>
    <form method="POST" action="datarbl.php">
        <input type="hidden" name="act" value="destroy">
        <input type="hidden" name="idrbl" value="<?= $idrbl ?>">
        ...
        <a href="datarbl.php" class="btn btn-secondary"> Batal </a>
    </form>
<?php
} else {
    echo "<script>window.location.href='datarbl.php';</script>";
}
?>