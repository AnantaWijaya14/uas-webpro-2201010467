...
$idrrq = $_GET["p1"];
$sql = "SELECT tu.nama,tu.nickname, tu.umur, tu.role , tu.jeniskelamin, tu.negara FROM tbrrq tu WHERE tu.idrrq='$idrrq';";
$hasil = mysqli_query($cnn, $sql);
if (mysqli_num_rows($hasil) > 0) {
    $h = mysqli_fetch_assoc($hasil);
?>
    <h3>Ubah Data Pemain <?= $h["nama"] ?></h3>
    <form method="POST" action="datarrq.php">
        <input type="hidden" name="act" value="update">
        <input type="hidden" name="idrrq" value="<?= $idrrq ?>">
        ...
        <a href="datarrq.php" class="btn btn-secondary"> Batal </a>
    </form>
<?php
} else {
    echo "<script>window.location.href='datarrq.php';</script>";
}
