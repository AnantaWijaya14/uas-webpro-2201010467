...
if (isset($_POST["act"])) {
    ...
    switch ($act) {
        case "store":
            ...
            $idrrq = md5($umur);
            $sql = "INSERT INTO tbrrq(nama, nickname, umur, role, jeniskelamin, negara, idrrq) VALUES('$nama','$nickname', '$umur', '$role', '$jeniskelamin','$negara', '$idrrq');";
        ...
        case "update":
            ...
            $negara = $_POST["txNEGARA"];
            $idrrq = $_POST["idrrq"];
            $sql = "UPDATE tbrrq SET nama='$nama', nickname='$nickname', umur='$umur', role ='$role' , jeniskelamin ='$jeniskelamin', negara = '$negara' WHERE idrrq='$idrrq';";
            $hasil = mysqli_query($cnn, $sql);
            if ($hasil) {
                $footer = "<script>
                        Swal.fire({
        ...
        case "destroy":
            $idrrq = $_POST['idrrq'];
            $sql = "DELETE FROM tbrrq WHERE idrrq='$idrrq';";
            $hasil = mysqli_query($cnn, $sql);
            if ($hasil) {
                $footer = "<script>
                        Swal.fire({
...
if (isset($_GET["aksi"])) {
    ...
    switch ($aksi) {
        case "new":
            $jdpage = "Tambah";
            $pg = "team/rrq/newrrq.php";
            break;
        case "edit":
            $jdpage = "Ubah";
            $pg = "team/rrq/editrrq.php";
            break;
        case "del":
            $jdpage = "Hapus";
            $pg = "team/rrq/delrrq.php";
            break;
        default:
            $jdpage = "List";
            $pg = "team/rrq/listrrq.php";
...
<head>
    ...
    <title><?= $jdpage ?> - Data Pemain RRQ</title>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Data Pemain RRQ</h1>
                </div>
                <?php
                include($pg);
                ?>
