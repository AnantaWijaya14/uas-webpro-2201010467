...
if (isset($_POST["act"])) {
    ...
    switch ($act) {
        case "store":
            ...
            $idrbl = md5($umur);
            $sql = "INSERT INTO tbrbl(nama, nickname, umur, role, jeniskelamin, negara, idrbl) VALUES('$nama','$nickname', '$umur', '$role', '$jeniskelamin','$negara', '$idrbl');";
        ...
        case "update":
            ...
            $negara = $_POST["txNEGARA"];
            $idrbl = $_POST["idrbl"];
            $sql = "UPDATE tbrbl SET nama='$nama', nickname='$nickname', umur='$umur', role ='$role' , jeniskelamin ='$jeniskelamin', negara = '$negara' WHERE idrbl='$idrbl';";
            $hasil = mysqli_query($cnn, $sql);
            if ($hasil) {
                $footer = "<script>
                        Swal.fire({
        ...
        case "destroy":
            $idrbl = $_POST['idrbl'];
            $sql = "DELETE FROM tbrbl WHERE idrbl='$idrbl';";
            $hasil = mysqli_query($cnn, $sql);
            if ($hasil) {
                $footer = "<script>
                        Swal.fire({
...
if (isset($_GET["aksi"])) {
    ...
    switch ($aksi) {
        case "new":
            $jdpage = "Tambah";
            $pg = "team/rbl/newrbl.php";
            break;
        case "edit":
            $jdpage = "Ubah";
            $pg = "team/rbl/editrbl.php";
            break;
        case "del":
            $jdpage = "Hapus";
            $pg = "team/rbl/delrbl.php";
            break;
        default:
            $jdpage = "List";
            $pg = "team/rbl/listrbl.php";
...
<head>
    ...
    <title><?= $jdpage ?> - Data Pemain rbl</title>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Data Pemain Rebellion</h1>
                </div>
                <?php
                include($pg);
                ?>
