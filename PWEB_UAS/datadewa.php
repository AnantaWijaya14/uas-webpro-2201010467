...
if (isset($_POST["act"])) {
    ...
    switch ($act) {
        case "store":
            ...
            $iddewa = md5($umur);
            $sql = "INSERT INTO tbdewa(nama, nickname, umur, role, jeniskelamin, negara, iddewa) VALUES('$nama','$nickname', '$umur', '$role', '$jeniskelamin','$negara', '$iddewa');";
        ...
        case "update":
            ...
            $negara = $_POST["txNEGARA"];
            $iddewa = $_POST["iddewa"];
            $sql = "UPDATE tbdewa SET nama='$nama', nickname='$nickname', umur='$umur', role ='$role' , jeniskelamin ='$jeniskelamin', negara = '$negara' WHERE iddewa='$iddewa';";
            $hasil = mysqli_query($cnn, $sql);
            if ($hasil) {
                $footer = "<script>
                        Swal.fire({
        ...
        case "destroy":
            $iddewa = $_POST['iddewa'];
            $sql = "DELETE FROM tbdewa WHERE iddewa='$iddewa';";
            $hasil = mysqli_query($cnn, $sql);
            if ($hasil) {
                $footer = "<script>
                        Swal.fire({
...
if (isset($_GET["aksi"])) {
    ...
    switch ($aksi) {
        case "new":
            $jdpage = "Tambah";
            $pg = "team/dewa/newdewa.php";
            break;
        case "edit":
            $jdpage = "Ubah";
            $pg = "team/dewa/editdewa.php";
            break;
        case "del":
            $jdpage = "Hapus";
            $pg = "team/dewa/deldewa.php";
            break;
        default:
            $jdpage = "List";
            $pg = "team/dewa/listdewa.php";
...
<head>
    ...
    <title><?= $jdpage ?> - Data Pemain dewa</title>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Data Pemain DEWA</h1>
                </div>
                <?php
                include($pg);
                ?>
